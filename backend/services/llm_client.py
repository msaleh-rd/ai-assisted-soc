...
class HypothesisItem(BaseModel):
    """A single competing root-cause hypothesis generated by the Investigation Swarm (Phase I)."""
    hypothesis: str = Field(description="One-sentence statement of this candidate root cause/attack narrative")
    supporting_techniques: List[str] = Field(default_factory=list, description="MITRE ATT&CK technique IDs consistent with this hypothesis")
    contradicting_signals: List[str] = Field(default_factory=list, description="Evidence/entities that would argue against this hypothesis")
    confidence: float = Field(description="The LLM's own confidence in this hypothesis, 0.0-1.0", ge=0.0, le=1.0)


class SwarmHypothesesOutput(BaseModel):
    """Structured output from the Investigation Swarm's parallel hypothesis generation."""
    hypotheses: List[HypothesisItem] = Field(description="3-5 competing, mutually distinct hypotheses for the investigation's root cause")
...
def validate_triage_output(output: "TriageOutput") -> List[str]:
    """Validate a TriageOutput against basic safety/sanity constraints (Phase D, Step 4).

    Checks severity is an allowed enum value, confidence is within [0.0, 1.0], and
    entity types are recognized. Returns a list of violation descriptions; empty
    if the output is valid.
    """
    violations: List[str] = []
    allowed_severities = {"critical", "high", "medium", "low"}
    if str(output.severity).strip().lower() not in allowed_severities:
        violations.append(f"severity '{output.severity}' not in {sorted(allowed_severities)}")

    if not (0.0 <= output.confidence <= 1.0):
        violations.append(f"confidence {output.confidence} outside [0.0, 1.0]")

    allowed_entity_types = {"user", "host", "ip", "file", "process", "domain", "hash", "url", "email"}
    for ent in output.entities_identified:
        if str(ent.type).strip().lower() not in allowed_entity_types:
            violations.append(f"entity type '{ent.type}' not in {sorted(allowed_entity_types)}")
