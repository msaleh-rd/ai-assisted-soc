...
start "SOC Temporal Worker" cmd /c "set USE_TEMPORAL=true&& set TEMPORAL_HOST=localhost:7233&& set DATABASE_URL=postgresql://soc_user:soc_password@127.0.0.1:5435/soc_platform&& .venv\Scripts\python.exe -m backend.services.temporal_worker"

REM 3. Start the local FastAPI backend
echo Starting local FastAPI API...
start "SOC API" cmd /c "set USE_TEMPORAL=true&& set TEMPORAL_HOST=localhost:7233&& set DATABASE_URL=postgresql://soc_user:soc_password@127.0.0.1:5435/soc_platform&& set PYTHONIOENCODING=utf-8&& .venv\Scripts\uvicorn.exe backend.main:app --host 0.0.0.0 --port 8000"
